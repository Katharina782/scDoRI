...
#' @export
addKathiGeneScoreMatrix <- function(
  input = NULL,
  ...
  .logThis(mget(names(formals()),sys.frame(sys.nframe())), ".addKathiGeneScoreMat Input-Parameters", logFile = logFile)
...
.addKathiGeneScoreMat <- function(
  ...
  if(useTSS){
    .logMessage(paste0(sampleName, " .addKathiGeneScoreMat useTSS = TRUE"))
    ...
    .logMessage(paste0(sampleName, " .addKathiGeneScoreMat useTSS = FALSE"))
    distMethod <- "GeneBody"
    geneRegions$geneStart <- start(resize(geneRegions, 1, "start"))
    geneRegions$geneEnd <- start(resize(geneRegions, 1, "end"))
    geneRegions <- extendGR(gr = geneRegions, upstream = geneUpstream, downstream = geneDownstream)
  ...
  .logThis(geneRegions, paste0(sampleName, " .addKathiGeneScoreMat geneRegions"), logFile = logFile)
  ...
  totalGS <- .safelapply(seq_along(geneRegions), function(z){
    ...
    totalGSz <- tryCatch({
      ...
      .logError(e, fn = ".addKathiGeneScoreMat TmpGS", info = sampleName, errorList = errorList, logFile = logFile)
  ...
  .logThis(featureDF, paste0(sampleName, " .addKathiGeneScoreMat FeatureDF"), logFile = logFile)
  ...
  for(z in seq_along(geneRegions)){
    ...
    o <- tryCatch({
      ...
      .logError(e, fn = ".addKathiGeneScoreMat AddToArrow", info = sampleName, errorList = errorList, logFile = logFile)
