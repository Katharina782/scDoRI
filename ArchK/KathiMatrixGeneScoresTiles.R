...
#' @export
addTADGeneScoreMatrix <- function(
  input = NULL,
  genes = getGenes(input),
  peaks = getPeakSet(input),
  ...
  .validInput(input = peaks, name = "peaks", valid = c("GRanges"))
  .validInput(input = tad_boundaries, name = "tadboundaries", valid = c("GRanges"))
  ...
  .logThis(mget(names(formals()),sys.frame(sys.nframe())), ".addTADGeneScoreMat Input-Parameters", logFile = logFile)

  #Valid GRanges
  genes <- .validGRanges(genes)
  peaks <- .validGRanges(peaks)
  tadBoundaries <- .validGRanges(tadBoundaries)
...
.addTADGeneScoreMat <- function(
  ...
  ## TAD
  seqlevels(tadBoundaries) <- as.character(unique(seqnames(tadBoundaries)))
  geneRegions <- geneRegions[!is.na(mcols(geneRegions)$symbol)]

  #Create Gene Regions Then Remove Strand Column
  
  # if we use TSS, we will resize the gene annotations to only include the TSS
  if(useTSS){
    .logMessage(paste0(sampleName, " .addTADGeneScoreMat useTSS = TRUE"))
    ...
    geneRegions <- resize(geneRegions, 1, "start") # resize to only start coordinate
    ...
    # if we consider the size of the gene body, we will keep start of the 
    # gene body as is, but we extend the start coordinate 5kb upstream
    # additionally we use a weight for larger gene bodys, so they don't bias the scores
  }else{
    .logMessage(paste0(sampleName, " .addTADGeneScoreMat useTSS = FALSE"))
    distMethod <- "GeneBody"
    geneRegions$geneStart <- start(resize(geneRegions, 1, "start"))
    geneRegions$geneEnd <- start(resize(geneRegions, 1, "end"))
    geneRegions <- extendGR(gr = geneRegions, upstream = geneUpstream, downstream = geneDownstream) # extend start -5kb upstream
    m <- 1 / width(geneRegions) # weight for larger genes
    geneRegions$geneWeight <- 1 + m * (geneScaleFactor - 1) / (max(m) - min(m)) # weight for larger genes
    # For the peakRegions, we only want to keep the start and later compute the 
    # peak middle coordinate
    peakRegions$peakStart <- start(resize(peakRegions, 1, "start"))
    peakRegions$peakEnd <- end(resize(peakRegions, 1, "start"))
  }
  ...
  ## TAD
  tadBoundaries <- sort(sortSeqlevels(tadBoundaries), ignore.strand = TRUE)
  
  
  .logThis(geneRegions, paste0(sampleName, " .addTADGeneScoreMat geneRegions"), logFile = logFile)
  #.logThis(peakRegions, paste0(sampleName, " .addGeneScoreMat peakRegions"), logFile = logFile)
  ...
  ## TAD
  tadBoundaries <- split(tadBoundaries, seqnames(tadBoundaries))
  tadBoundaries <- lapply(tadBoundaries, function(x){
    mcols(x)$idx <- seq_along(x)
    return(x)
  })
  ...
  totalGS <- .safelapply(seq_along(geneRegions), function(z){
    ...
    totalGSz <- tryCatch({
      ...
      ## TAD
      tadboundz <- tadBoundaries[[z]]
      tadboundz <- tadboundz[order(tadboundz$idx)]
      chrt <- paste0(unique(seqnames(tadboundz)))
      stopifnot(chrz == chrt)
      
      subset_peak_mat <- copy(peak_mat)
      subset_peak_mat <- subset_peak_mat[rownames(subset_peak_mat) %in% names(peakRegionz), ]
      
      # create middle of each peak
      ...
      if(useGeneBoundaries){
        stop("Error use Gene Boundaries not recommended when considering TAD boundaries")
       ...
       # rm(s, e, pReverse, pReverseMin, pForward, pForwardMin, geneStartz, geneEndz, pminGene, pmaxGene)
        ...
        ## TAD
        for (i in seq.int(length(geneRegionz))){
          overlap <- findOverlaps(geneRegionz[i], tadboundz)
          if (length(overlap) == 0){
            geneRegionz[i]$geneStart = start(geneRegionz[i])
            geneRegionz[i]$geneEnd = end(geneRegionz[i])
          }
          #stopifnot(length(subjectHits(overlap)) <= 2)
          else if (length(subjectHits(overlap)) == 1){
            geneRegionz[i]$geneStart = start(tadboundz[subjectHits(overlap)])
            geneRegionz[i]$geneEnd = end(tadboundz[subjectHits(overlap)])
          } else {
            geneRegionz[i]$geneStart = min(start(tadboundz[subjectHits(overlap)[1]]),
                                           start(tadboundz[subjectHits(overlap)[2]]))
            geneRegionz[i]$geneEnd = max(end(tadboundz[subjectHits(overlap)[1]]), 
                                         end(tadboundz[subjectHits(overlap)[2]]))
            
            
          }
        }
        
        extendedGeneRegion <- geneRegionz
        start(ranges(extendedGeneRegion)) <- extendedGeneRegion$geneStart
        end(ranges(extendedGeneRegion)) <- extendedGeneRegion$geneEnd
      }

      tmp <- suppressWarnings(findOverlaps(extendedGeneRegion, GRanges(peakRegionz)))
      ...
      peak_middle_region <- resize(peak_middle_region, width = 1, fix = "start")


      
      x <- distance(ranges(geneRegionz)[queryHits(tmp)], 
                    ranges(peak_middle_region)[subjectHits(tmp)])
      ...
      #Clean Memory
      rm(isMinus, signDist, extendedGeneRegion, uniquePeaks)
      rm(matGS, tmp)
      gc()
      ...
      .logError(e, fn = ".addTADGeneScoreMat TmpGS", info = sampleName, errorList = errorList, logFile = logFile)
  ...
  .logThis(featureDF, paste0(sampleName, " .addTADGeneScoreMat FeatureDF"), logFile = logFile)
  ...
  for(z in seq_along(geneRegions)){
    ...
    o <- tryCatch({
      ...
      .logError(e, fn = ".addTADGeneScoreMat AddToArrow", info = sampleName, errorList = errorList, logFile = logFile)
