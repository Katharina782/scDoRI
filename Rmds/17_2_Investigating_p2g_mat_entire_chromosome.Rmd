...
p2g_join %>%
  ...
  mutate(rolling_avg = zoo::rollmean(Correlation, 6500,
                                                align = "left", fill = NA)) %>%
  drop_na() %>%
  ggplot(aes(x = distance, y = rolling_avg)) +
  geom_line()
```
...
p2 <- ggplot() + geom_histogram(aes(x = width(tad_boundaries)), bins = 125) +
...
ggsave("/omics/groups/OE0533/internal/katharina/scDoRI/gastrulation_data_new/Kathi/plots/new_tad_distribution.pdf", height = 6, width = 10)
```
