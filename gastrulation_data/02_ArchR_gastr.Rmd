...
```{r}
proj <- loadArchRProject("07_ArchRobject_all_timepoints_dimred/")
```
...
```{r}
tibble(gene = rowData(gene_scores)$idx,
...
lognorm_rna <- as.matrix(log(norm_rna * sc_factor + 1))
```
...
test <- cor(lognorm_rna["Lamb1", ], gene_score_mat["Lamb1", ])
```
