...
totalGS <- ArchR:::.safelapply(seq_along(geneRegions), function(z){ # we loop over the z chromosomes here
  ...
  totalGSz <- tryCatch({
    ...
    print(paste0("In total there are ", subjectLength(tmp), " overlapping with gene regions found on this chromosome"))
    ...
    x <- distance(ranges(geneRegionz)[queryHits(tmp)], ranges(peakRegionz)[subjectHits(tmp)])
