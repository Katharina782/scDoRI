...
seurat_objects <- map(stages, function(n){
  ...
  gastr_subset <- gastr_subset %>%  RunPCA(features = VariableFeatures(gastr_subset), verbose = FALSE) %>% 
    ...
    RunTSNE(verbose = TRUE, dims = 1:15)
...
```{r}
tsnes <- comprehenr::to_list(for (i in 1:3)
  DimPlot(gastr_seurat, reduction = "pca", dims = i:(i+1)) +
    theme())
```
...
plots <- map(seq.int(1,3), function(n){
  ...
  p1 <- DimPlot(object, reduction = "tsne", group.by = "celltype.mapped", 
                cols = col, size = .1) +
...
```{r}
atac_seurat <- RunTSNE(atac_seurat_test, reduction = "lsi", dims = 2:30)
...
```{r, fig.width = 15, fig.height=10}
DimPlot(atac_seurat, group.by = "celltype.mapped", size = .1, cols = col, reduction = "tsne") +
  labs(title = "scATAC-seq Celltype")
```

```{r, fig.width=15, fig.height=10}
DimPlot(gastr_seurat , group.by = "celltype.mapped", size = .1, cols = col) +
