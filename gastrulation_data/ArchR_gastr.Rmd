...
proj <- loadArchRProject(path = "FirstQC_ArchRobject/")
```
...
grid.arrange(grobs = beforeQC, ncol = 2)
...
grid.arrange(grobs = samples, ncol = 2)
...
grid.arrange(grobs = doublet_enrichment, ncol = 2)
...
grid.arrange(grobs = doublet_density, ncol = 2)
```
...
In total after QC and doublet filtering the dataset contains 59,113 cells. First, 
we have to remove the cells from E8.75_rep1/2, because these samples are
not included in the processed and annotated data. 8,880 cells are from samples 
E8.75_rep1/2. 
In the processed data matrix we find 38,158 cells. Only 30,803 cells pass RNA 
QC. Of the scRNA-seq dataset only 30,282 out of 36,822 cells pass the scATAC-seq
QC. This means that we have 30,282 cells with annotated celltypes. These are 
the cells we want to use in the following analysis. We will, therefore, subset
our dataset. We are now left with 28,060 cells for further analysis. Apparently 
we filtered some cells not fitlered in the processed data.


```{r}
# read in processed scATAC-seq data
processed_atac <- readH5AD("anndata_atac.h5ad")
processed_atac

remove <- (as.data.frame(colData(processed_atac)) %>% rownames_to_column("cell") %>% 
                          filter(pass_rnaQC == TRUE))[["cell"]]

processed_atac <- processed_atac[ , colnames(processed_atac) %in% remove]

# check the RNA dataset as well to see if the cells overlap with the ATAC cells
proc_rna <- readH5AD("anndata_rna.h5ad")

#
keep <- colnames(proc_rna[,colnames(proc_rna) %in% colnames(processed_atac)])
```
...
We will add the celltype annotation from the processed data to our ArchR project.

```{r}
filt_proj <- addTSNE(ArchRProj =filt_proj, reducedDims = "IterativeLSI")

filt_proj <- addUMAP(ArchRProj =filt_proj, reducedDims = "IterativeLSI")
```
...
include <- colnames(processed_atac)[colnames(processed_atac) %in% filt_proj$cellNames]

filtered_processed_atac <- as.data.frame(colData(processed_atac)) %>% rownames_to_column("cell") %>% 
  filter(cell %in% include)

celltypes <- as.character(filtered_processed_atac$celltype.mapped)

names(celltypes) <- filtered_processed_atac$cell
filt_proj <- addCellColData(filt_proj,
               data = celltypes, 
               name = "celltypes",
               cells = as.character(filtered_processed_atac$cell)
               )
```
...
```{r}
#saveArchRProject(filt_proj, outputDirectory = "annotated_ArchRobject", load = FALSE)

#filt_proj <- loadArchRProject(path = "annotated_ArchRobject/")
...
```{r}
filt_proj <- addImputeWeights(filt_proj)
```
...
Filter genes which are found in gene score matrix

Reorder cell names to match

```{r}
# Try to reorder the cellnames
...
Could it be that the gene annotations are different between the ArchR object and
the RNA data?
...
"IterativeLSI". We will call the one for the RNA data "LSI_RNA"
