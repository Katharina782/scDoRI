...
normalization_df %>% ggplot(aes(x = total_sites, y = total_activity)) + geom_point() +
  labs(title = "Before linear regression")
...
We can compute the size factors in two different ways. The first way is as described
in the methods section of the Cicero paper (https://www.sciencedirect.com/science/article/pii/S1097276518305471?via%3Dihub#sec4).
Here, they use the output of the linear model to scale the accessibility we computed
above.

$R_{ki}= R_{ki} * \frac{\sum_{i}{r_i}}{r_i}$
