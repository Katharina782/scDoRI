...
```{r}
new_coldata <- getCellColData(proj) %>% as.data.frame() %>% unite(time_celltype, Sample, celltypes, sep = "_")
...
proj <- addCellColData(
  ...
  name = "timepoint_celltype", 
  cells = rownames(new_coldata), 
  force = FALSE
)

getCellColData(proj) %>% as.data.frame() %>% group_by(timepoint_celltype) %>% 
  summarise(n = n())
```
...
```{r}
markerPeaks_timepoint <- getMarkerFeatures(  ArchRProj = subset_proj,
  groupBy = "timepoint_celltype",
  useMatrix = "PeakMatrix", 
  bias = c("TSSEnrichment", "log10(nFrags)"),
  testMethod = "wilcoxon",
  maxCells = 500,
