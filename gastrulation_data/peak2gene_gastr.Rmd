...
```{r}
# load ArchR project
proj <- loadArchRProject(path = "03_added_gene_expression_ArchRobject/")
...
```{r}
peak_set <- ArchR::getPeakSet(proj) %>%
  tibble::as_tibble()
print(paste0("Dimensions of the peak set are: ", paste(dim(peak_set), collapse=", ")))
```
...
```{r}
#saveArchRProject(filt_proj, outputDirectory = "04_peak_calling_ArchRobject", load = FALSE)
```
