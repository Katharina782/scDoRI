...
p2g_matrix_from_df <- function(p2g, metadata_p2g_rna, metadata_p2g_atac){
  ...
  p2g_idx <- left_join(rna_idx, p2g_idx, by = "idxRNA", copy = TRUE )
...
writeH5AD(sce, "/omics/groups/OE0533/internal/katharina/scDoRI/gastrulation_data/jupyter_notebooks/p2g_gene_activity_scores/debugging",
          X_name = "scores")
...
writeH5AD(sce, "/omics/groups/OE0533/internal/katharina/scDoRI/gastrulation_data/jupyter_notebooks/pos_p2g_links_scores", X_name = "scores")
```
</details>
...
p4 <- p2g_join %>%  
  ...
  labs(title = "Relationship between distance and correlation of p2g links, 1kb bins",
       x = "Distance < 100kb", y = "Correlation") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
### PLOT
ggplot() + geom_histogram(aes(x = distance), bins = 200) +
...
#### PLOT
print(ggplot() + geom_histogram(aes(x = distance), bins = 500) + 
